    ...
    Private Const MinOcc As Double = 0.8
        ...
        Private Sub InitLogging()
            ...
            File.WriteAllText(_decisionLogPath,
                              "RunId,Phase,OpRec,OrderNo,OpNo,ResGroup,ChosenRes,PlanDay,Start,Finish,BatchOrCartId,KlinType,CycleType,Occ,Score,Reason" & Environment.NewLine)

            File.WriteAllText(_scheduleExportPath,
                              "RunId,OpRec,OrderNo,OpNo,OpName,ResGroup,ChosenRes,PlanDay,Start,Finish,BatchOrCartId,KlinType,CycleType,Occ" & Environment.NewLine)
        End Sub
        ...
        Private Class OpInfo
            ...
            Public Property KlinType As String    ' "Batch" or "Tunnel"
        ...
        Private Sub ApplyFiringPlan(preactor As IPreactor,
            ...
            Dim batchOps = op300.Where(Function(o) o.KlinType.Equals("Batch", StringComparison.OrdinalIgnoreCase)).ToList()
            Dim tunnelOps = op300.Where(Function(o) o.KlinType.Equals("Tunnel", StringComparison.OrdinalIgnoreCase)).ToList()
            ...
            Dim resRecs = planningboard.FindResources(op290Rec)
                ...
                For Each resRec In resRecs
                    ...
                    LogDecision("FIRING_PUT290", op290Rec, op300Info.OrderNo, 290, "(LOAD)", resName, day,
                                startTime, TryGetApproxFinish(test.Value), batchOrCartId, op300Info.KlinType, cycleType, op300Info.VolumeOcc, 0,
                ...
                If Not placed290 Then
                    LogDecision("FIRING_SKIP290", op290Rec, op300Info.OrderNo, 290, "(LOAD)", "", day,
                                Nothing, Nothing, batchOrCartId, op300Info.KlinType, cycleType, op300Info.VolumeOcc, 0,
        ...
        Dim op300Rec As Integer = op300Info.OpRec
            ...
            For Each resRec In resRecs300
                ...
                ExportPlannedRow(op300Rec, op300Info.OrderNo, 300, op300Info.OpName, op300Info.ResGroup, resName, day,
                                 startTime, finishApprox.Value, batchOrCartId, op300Info.KlinType, cycleType, op300Info.VolumeOcc)
            ...
            While opRec > 0
                ...
                For Each resRec In resRecs
                    ...
                    LogDecision("FWD_PUT", nextRec, info.OrderNo, info.OpNo, info.ResGroup, resName, DateTime.Today,
                                test.Value.ChangeStart, TryGetApproxFinish(test.Value), "", info.KlinType, info.CycleType, info.VolumeOcc, 0,
