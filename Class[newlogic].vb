...
Public Class CsvRoutingReader
    ...
    '===========================================================
    ' FUNCTION NAME : AddExpectedFiringStartDate
    ...
    '        Throw New Exception("Invalid occupancy limits. Ensure: 0 <= minOccupancy <= maxOccupancy and maxOccupancy > 0")
    '    End If
    ...
    '    End If
    '    If Not routingTable.Columns.Contains(kilnTypeCol) Then
    '        Throw New Exception("Required column missing: " & kilnTypeCol)
    '    End If
    '    If Not routingTable.Columns.Contains(cycleTypeCol) Then
    '        Throw New Exception("Required column missing: " & cycleTypeCol)
    '    End If

    '    If Not routingTable.Columns.Contains(orderNoCol) Then
    '        Throw New Exception("Required column missing for occupancy batching: " & orderNoCol)
    '    End If
    '    If Not routingTable.Columns.Contains(operationNoCol) Then
    '        Throw New Exception("Required column missing for occupancy batching: " & operationNoCol)
    '    End If
    '    If Not routingTable.Columns.Contains(volOccCol) Then
    '        Throw New Exception("Required column missing for occupancy batching: " & volOccCol)
    '    End If

    '    '-------------------------------------------------------
    '    ' Add output columns if needed
    '    '-------------------------------------------------------
    '    If Not routingTable.Columns.Contains(firingWeekCol) Then
    '        routingTable.Columns.Add(firingWeekCol, GetType(Integer))
    '    End If
    '    If Not routingTable.Columns.Contains(weekStartCol) Then
    '        routingTable.Columns.Add(weekStartCol, GetType(String))
    '    End If
    '    If Not routingTable.Columns.Contains(cycleBatchCol) Then
    '        routingTable.Columns.Add(cycleBatchCol, GetType(String))
    '    End If

    '    If Not routingTable.Columns.Contains(batchOccupancyCol) Then
    '        routingTable.Columns.Add(batchOccupancyCol, GetType(Double))
    '    End If
    '    If Not routingTable.Columns.Contains(batchOrdersCol) Then
    '        routingTable.Columns.Add(batchOrdersCol, GetType(String))
    '    End If
    ...
    '    '=======================================================
    '    ' STEP B: Build Order -> Occupancy map from operation 300
    '    '=======================================================
    ...
    '        Dim occ As Double
    '        If Not Double.TryParse(occStr, NumberStyles.Any, CultureInfo.InvariantCulture, occ) Then Continue For

    '        orderToOcc(orderNo) = occ
    '    Next

    '    '=======================================================
    '    ' STEP C: Build order-level list for grouping
    '    '=======================================================
    ...
    '        Dim cycleType As String = row(cycleTypeCol).ToString().Trim()
    '        If String.IsNullOrWhiteSpace(kilnType) OrElse String.IsNullOrWhiteSpace(cycleType) Then Continue For

    '        Dim expectedDateStr As String = row(expectedDateCol).ToString().Trim()
    '        Dim expectedDate As DateTime
    '        If Not DateTime.TryParseExact(expectedDateStr, dateFormat, culture, DateTimeStyles.None, expectedDate) Then Continue For

    '        If Not orderToOcc.ContainsKey(orderNo) Then Continue For
    ...
    '    '=======================================================
    '    ' STEP D: Greedy batching + store metadata using UNIQUE batch instance keys
    '    '=======================================================

    '    ' Visible label: CycleType_BatchNo (unchanged)
    '    Dim orderToCycleBatch As New Dictionary(Of String, String)(StringComparer.OrdinalIgnoreCase)

    '    ' FIX: Map each order to the UNIQUE batch instance key
    '    Dim orderToBatchInstanceKey As New Dictionary(Of String, String)(StringComparer.OrdinalIgnoreCase)

    '    ' FIX: Store batch totals/orders by UNIQUE batch instance key
    ...
    '    For Each weekGroup In grouped

    '        Dim kilnGroups = weekGroup.GroupBy(Function(o) o.KilnType)

    '        For Each kilnGroup In kilnGroups

    '            Dim cycleGroups = kilnGroup.GroupBy(Function(o) o.CycleType)
    ...
    '                    Dim currentBatch As New List(Of OrderInfo)()
    '                    Dim totalOcc As Double = 0.0

    '                    ' Seed with earliest remaining order
    '                    Dim seed As OrderInfo = remaining(0)
    '                    currentBatch.Add(seed)
    '                    totalOcc += seed.Occupancy
    '                    remaining.RemoveAt(0)

    '                    ' Fill candidates: prefer larger occupancy that fits under max
    '                    Dim fillCandidates = remaining.OrderByDescending(Function(o) o.Occupancy).ToList()

    '                    For Each candidate In fillCandidates
    '                        If totalOcc >= maxOccupancy Then Exit For
    '                        If totalOcc + candidate.Occupancy <= maxOccupancy Then
    '                            currentBatch.Add(candidate)
    '                            totalOcc += candidate.Occupancy
    '                        End If
    '                    Next

    '                    ' Remove picked from remaining
    '                    For Each picked In currentBatch
    '                        remaining.RemoveAll(Function(o) o.OrderNo.Equals(picked.OrderNo, StringComparison.OrdinalIgnoreCase))
    '                    Next

    '                    ' Visible label remains unchanged
    '                    Dim batchLabel As String = cycleGroup.Key & "_" & batchNo.ToString()

    '                    ' FIX: Unique batch instance key prevents overwrites across weeks/kilns/cycles
    '                    Dim batchInstanceKey As String =
    '                            weekGroup.Key & "|" & kilnGroup.Key & "|" & cycleGroup.Key & "|" & batchNo.ToString()

    '                    ' Assign mappings
    '                    Dim orderList As New List(Of String)()
    '                    For Each picked In currentBatch
    '                        orderToCycleBatch(picked.OrderNo) = batchLabel
    '                        orderToBatchInstanceKey(picked.OrderNo) = batchInstanceKey
    '                        orderList.Add(picked.OrderNo)
    '                    Next

    '                    ' Store metadata against unique instance key
    '                    batchToTotalOcc(batchInstanceKey) = totalOcc
    '                    batchToOrders(batchInstanceKey) = orderList
    ...
    '    '=======================================================
    '    ' STEP E: Write results back to ALL rows per order
    '    '=======================================================
    '    For Each row As DataRow In routingTable.Rows
    ...
    '        ' Visible cycle+batch
    '        If orderToCycleBatch.ContainsKey(orderNo) Then
    '            row(cycleBatchCol) = orderToCycleBatch(orderNo)
    '        End If

    '        ' batch+occupancy for that exact batch instance
    '        If batchToTotalOcc.ContainsKey(batchInstanceKey) Then
    '            row(batchOccupancyCol) = batchToTotalOcc(batchInstanceKey)
    '        End If

    '        ' batch+orders list for that exact batch instance
    '        If batchToOrders.ContainsKey(batchInstanceKey) Then
